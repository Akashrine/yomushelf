-- Hotfix: apply everything that failed in 20240002000000 due to existing trigger

-- 1. Recreate trigger safely
drop trigger if exists profiles_sync_public_slug on public.profiles;
...
-- 2. ENUMs
do $$ begin
  create type public.share_type as enum ('profile_link', 'collection_card', 'wrapped_card');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.share_format as enum ('landscape', 'story', 'link');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.share_platform as enum ('x', 'tiktok', 'instagram', 'whatsapp', 'discord', 'direct', 'copy');
exception when duplicate_object then null; end $$;
...
create index if not exists wrapped_snapshots_user_id_idx on public.wrapped_snapshots (user_id);
...
grant execute on function public.generate_wrapped(uuid, integer) to service_role;
